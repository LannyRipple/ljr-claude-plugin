...
**Detecting whether you're inside tmux:** In Bash tool subprocesses, `$TMUX` is overridden
to point to Claude's isolated socket and is always set — do not use it to detect whether
the user is in tmux. Instead check `$TMUX_PANE`: it is inherited from the user's tmux
session at Claude startup and will be non-empty only if the user started Claude from inside
tmux. Don't rely on running `tmux display-message` as a test — it will succeed against
Claude's own socket even if the user is not in tmux.
...
# Find billboard dynamically (scoped to this window via $TMUX_PANE)
...
```bash
# show-and-go — opens above (billboard position), wait for Enter, then pane closes
...
# show-and-shell — opens above, output then interactive shell
tmux split-window -bv 'some-command 2>&1; exec bash'

# show-and-watch — opens above, live-updating monitor, Ctrl-C to kill (use -d, focus stays with user)
...
- `remain-on-exit on` is a fallback if the primary approach fails — it leaves a `[dead]` pane the user must close manually, so prefer `read`/`exec bash`/`watch`
