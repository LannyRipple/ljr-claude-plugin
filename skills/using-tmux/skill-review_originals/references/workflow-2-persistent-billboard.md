...
```bash
tmux list-panes -F "#{pane_index}: cmd=#{pane_current_command} dead=#{pane_dead}"
```
...
- If the pane dies (e.g. file deleted out from under it), use `respawn-pane -k -t .$BILLBOARD 'tail -f ~/tmp/radar-display.txt'`
- Check pane health: `tmux list-panes -F "#{pane_index}: #{pane_current_command} dead=#{pane_dead}"`
