...
Use `$TMUX_PANE` (set by tmux in every pane's environment) to scope the lookup to this
Claude's window. Multiple Claude instances may run in separate windows — always scope.
...
- `display-message -I` requires the pane to be truly empty (no running process) — errors with "pane is not empty"
